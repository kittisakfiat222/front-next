...
const UserPage =  ({ getuser }: UserProps) =>  {
  ...
  const [formData, setFormData] = useState({
    ...
    tel: getuser.user.tel || '',
    role: getuser.user.role || 'User',
  });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL}/manageuser/${getuser.user.id}`, {
        method: 'PUT',
        headers: {
    ...
    <Layout>
      <Container maxWidth="sm">
        <Card sx={{ maxWidth: 345 }}>
          <CardMedia
            sx={{ height: 140 }}
            image={getuser.user.avatar}
            title={`${getuser.user.fname} ${getuser.user.lname}`}
          />
          <CardContent>
            <Typography gutterBottom variant="h5" component="div">
              {getuser.user.fname} {getuser.user.lname}
            </Typography>
